...
export default function ViewTimetables({ navigate }) {
  ...
  const currentEntries = useMemo(() => {
    ...
    return []
  }, [state.timetable, selectedId, viewType])
  ...
  const viewOptions = (viewType === 'class' && sectionFilter)
    ...
    : viewType === 'class' ? state.classes : state.teachers
  const bulkSourceList = (bulkType === 'classes' && sectionFilter)
    ? state.classes.filter(c => c.sectionId === sectionFilter)
    : bulkType === 'classes' ? state.classes : state.teachers
  ...
  return (
    <div className="p-8">
      ...
      <PageHeader
        ...
        subtitle="View and export timetables as PDF or CSV"
        action={periodLabel && (
      ...
      <div className="mb-4">
        <Tabs
          ...
          active={bulkMode ? 'bulk' : 'single'}
          onChange={(id) => { setBulkMode(id === 'bulk'); sounds.click() }}
        />
      </div>
        ...
        <div>
          ...
          <Card className="p-4 mb-6">
            <div className="flex flex-wrap items-end gap-4">
              <div>
                ...
                <select
                  value={viewType}
                  onChange={e => { setViewType(e.target.value); setSelectedId(''); sounds.click() }}
                  className="px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 bg-white"
                >
          ...
          {viewType !== 'master' && (
              ...
              <div className="ml-auto flex items-end gap-4">
                ...
                <Button variant="secondary" onClick={handleExportCSV} disabled={viewType !== 'master' && !selectedId}>
                  Export Spreadsheet
                </Button>
                <Button onClick={handleExportPDF} disabled={viewType !== 'master' && !selectedId}>
                  Export PDF
                </Button>
              </div>
            </div>
