...
export function exportMasterPDF({ days, periods, timetables, paperSize, orientation, school, periodLabel, title }) {
    ...
    for (const day of days) {
      ...
      if (entries.length === 0) {
        ...
        const cellText = entries.map(e => {
          let line = `${e.className || ''}: ${e.subjectName || ''} - ${e.teacherName || ''}`
  ...
  const gridAvailH = pageH - gridStartY - 10 * scale
  const gridRowH = Math.min(gridAvailH / (gridBody.length + 1), 20 * scale)
  ...
  autoTable(pdf, {
    ...
    columnStyles: {
      0: { fontStyle: 'bold', cellWidth: 28 * scale, fillColor: [219, 234, 254], fontSize: (isLandscape ? 6 : 5) * scale },
    },
    margin: { left: margin, right: margin },
  ...
  for (const teacher of teachers) {
      ...
      for (const p of allPeriods) {
        ...
        if (entries.length === 0) {
          ...
          const cellText = entries.map(e => {
            ...
            if (isSecondary) {
              return `${e.secondaryClassName || ''}: ${e.secondarySubjectName || ''}`
            }
            let line = `${e.className || ''}: ${e.subjectName || ''}`
            if (e.roomName) line += `\n[${e.roomName}]`
            if (e.secondaryClassName) {
              line += `\n  → ${e.secondaryClassName}: ${e.secondarySubjectName || ''} (${e.secondaryTeacherName || ''})`
            }
    ...
    const availHeight = pageH - currentY - 10 * scale
    const rowHeight = Math.min(availHeight / (body.length + 1), 18 * scale)
    ...
    autoTable(pdf, {
      ...
      columnStyles: {
        0: { fontStyle: 'bold', cellWidth: 22 * scale, fillColor: [219, 234, 254] },
      },
      margin: { left: margin, right: margin },
      didDrawPage: addFooter,
    })
